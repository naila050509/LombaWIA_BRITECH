...
const DRAWER_W = 256; // w-64 = 16rem = 256px
...
const Navbar = ({ textColor = "text-black" }) => {
  ...
  // ✅ auto close saat pindah route (biar ga nyangkut)
  useEffect(() => {
    setIsOpen(false);
    setShowSearch(false);
  }, [location.pathname]);
    ...
    <>
      ...
      <nav className="fixed top-0 left-0 w-full bg-white/70 backdrop-blur-md shadow-md z-[10000]">
        <div className="container py-4 flex justify-between items-center px-4">
              ...
              <AnimatePresence>
                {showSearch && (
                  <motion.div
                    ...
                    className="absolute right-0 top-10 bg-white/80 backdrop-blur-md shadow-lg border border-gray-200 rounded-full flex items-center px-4 py-2 w-72 z-[10001]"
                  >
          ...
          {/* Tombol Menu Mobile (hamburger only) */}
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Sidebar (drag hanya dari handle) */}
            <motion.aside
              initial={{ x: -DRAWER_W }}
              animate={{ x: 0 }}
              exit={{ x: -DRAWER_W }}
              transition={{ type: "tween", duration: 0.22 }}
              className="fixed top-0 left-0 h-dvh w-64 bg-white text-black z-[10001] shadow-2xl pointer-events-auto touch-pan-y"
              role="dialog"
              aria-modal="true"
              drag="x"
              dragControls={dragControls}
              dragListener={false} // ✅ penting: klik X/link ga ke-drag
              dragConstraints={{ left: -DRAWER_W, right: 0 }}
              dragElastic={0.06}
              onDragEnd={(e, info) => {
                if (info.offset.x < -80 || info.velocity.x < -700) {
                  setIsOpen(false);
                }
              }}
            >
                  ...
                  {NavbarMenu.map((menu, index) => (
                    <div key={menu.id}>
                      <Link
                        ...
                        className="block text-black hover:text-yellow-600 transition-colors text-lg font-medium"
                      >
                        {menu.title}
                      </Link>
